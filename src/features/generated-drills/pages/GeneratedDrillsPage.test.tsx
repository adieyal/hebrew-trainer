...
import { GeneratedDrillsPage } from "./GeneratedDrillsPage";

afterEach(async () => {
  await resetTrainerDb();
});

describe("GeneratedDrillsPage", () => {
  test("shows only generated drill entries", async () => {
    ...
    render(<GeneratedDrillsPage />);

    expect(await screen.findByText(/1 generated drills/i)).toBeInTheDocument();
    expect(screen.getAllByText(/first, i need to check with my spouse\./i)).toHaveLength(2);
    expect(screen.queryByText(/i need to check with my spouse first\./i)).not.toBeInTheDocument();
  });
});
